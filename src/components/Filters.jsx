...
export const Filters = ({ setPokemonsFilters, setPokemons, setPage }) => {
  ...
  useEffect(() => {
    const getPokemonsType = async () => {
      const { data } = await axios.get(`${url}/type/${selectTypeFilter}`);
    ...
    if (selectTypeFilter !== '') getPokemonsType();
  }, [selectTypeFilter, setPokemonsFilters, setPokemons, setPage]);

  useEffect(() => {
    const getPokemonsAbility = async () => {
      const { data } = await axios.get(`${url}/ability/${selectAbilityFilter}`);
    ...
    if (selectAbilityFilter !== '') getPokemonsAbility();
